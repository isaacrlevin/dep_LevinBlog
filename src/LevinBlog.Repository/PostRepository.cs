...
namespace LevinBlog.Repository
{
    ...
    public class PostRepository : IPostRepository
    {
        ...
        public IEnumerable<PostEntity> GetQueryPaged(string query, int count, int page = 1)
        {
            return _blogContext
                    .Posts
                    .Where(x => x.Published && (x.Title.ToLower().Contains(query.ToLower()) || x.Article.Content.ToLower().Contains(query.ToLower())))
                        .Include(x => x.Excerpt)
                        .Include(x => x.Category)
                        .Include(x => x.PostTags)
                            .ThenInclude(i => i.Tag)
        ...
        public IEnumerable<PostEntity> GetPostsBasedOnIdCollection(List<int> postIds)
        {
            return _blogContext
                    ...
                    .Include(x => x.Category)
                    .Include(x => x.Excerpt)
                    .OrderBy(d => postIds.IndexOf(d.Id)).ToList()
                    .ToList();
        }
        ...
        {
            return (from x in _blogContext.Posts where x.Id < id && x.Published orderby x.Id descending select x).FirstOrDefault();
        }
        ...
        {
            return (from x in _blogContext.Posts where x.Id > id && x.Published orderby x.Id select x).FirstOrDefault();
        }
        ...
        public void Update(Post post)
        {
            ...
            // Category
            entity.CategoryId = post.Category.Id;
